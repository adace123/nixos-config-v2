...
| Where | What |
| --- | --- |
| `K3` in the top rule | the id. Tinted while an agent has the card: yellow working, grey idle, dimmer still for one that exited. Blocked and done leave it alone — they already colour the whole border — and a live status outranks a stale age. |
...
delete (which stops its agent), `x` stop the agent but keep the card, `s` send,
...
the detail view tick that checklist entry. Clicking selects, double-clicking
opens the detail, the wheel scrolls a column; a column heading with `▴` or `▾`
beside its count has cards off screen either way.
...
The workspace label is never dropped, it truncates.
...
K3` for a card that came out of another one, and `spawned  K7 (Backlog)` for the
cards it produced. History is capped at the last 25 entries on disk and the last
12 are shown.

**Sending a card** (`s`) opens a form (editable prompt, workspace, agent kind,
agent name) and then: create a tab in the card's workspace, run
...
Where a send *puts* the card depends on where it came from: a card still in the
**backlog** was never committed to anything, so sending it queues it in
**Queued**; a card already in the flow lands in **In Progress**, which is where
the agent it just started belongs. Both are matched by column *id*
(`Config.send_column`), so a board that calls its columns
`later`/`next`/`started` gets the same rule — and `queued`, `todo`, `next` and
`ready` are all read as "the queued column" — while a board with neither column
leaves the card where it is. `wip_limits` are checked against the column the send
would land in.
...
agent and closes the tab its dispatch opened, as part of deleting. `x` does the
same without deleting the card, which is what you want for a runaway run. Both
confirm first, and both leave a tab alone if its label is no longer the one the
board set at dispatch — that pane is yours now, not the card's.
...
**When an agent blocks, herdr says so.** Blocked is the one state that costs
something to ignore — the agent is stopped until you answer — and both places
that show it require looking at the board. So the first time a card *arrives* in
that state, the board raises a notification (`K6 needs you`, with the title).
Only transitions: a board that opens onto a blocked card stays quiet, because
nothing has changed since you last looked. `notify_on_block = false` turns it
off.
...
```text
---
herdr kanban: this card is K3. Keep it current as you work:
...
id it has to carry around. Humans run the same verbs with an explicit id:
...
| `herdr-kanban send [<task>] [--agent <kind>] [--workspace <id>] [--model <name>] [--dry-run]` | start an agent for a card, without the board |
...
**A card knows where it came from.** `--from K3` records which card the work fell
...
K3` on the new card, `spawned  K7 (Backlog)` on the old one — and `list`
shows `· found during K3`. The reason is the one thing a follow-up loses the
...
the message names it (`K3 already covers this: "…" (Backlog)`) and points at
...
the board's add form never refuses you.

**`send`** is the board's `s`, for a script: `herdr-kanban send K5` dispatches
...
recorded on it, so a later re-send does not quietly go back to the default. It is
...
**Status rights.** Dispatch moves a backlog card to Queued and anything else to In
Progress (see above). An agent may set `doing`, `blocked`, `review` (or move a
card back to `queued`/`backlog` if it turns out not to be actionable), but
**`done` is refused** by the CLI — the message points at `--force`, and the
intention is that only you close a card. This keeps Done trustworthy and mirrors
the convention on the `tsk` board. The guard is on the CLI only: in the board
itself `H`/`L` move a card to any column, so you are never fighting it.
...
`herdr-kanban title K3 "…"` from a shell) the card is yours: a later agent title
is recorded under **Updates** as `suggested title: …` instead of overwriting
you, and `✎` disappears. The capture title is kept in `original_title` either
way and the detail view shows it, so a rename is always reversible. Pass
`--force` to let an agent override you deliberately.
...
- **Labels and workspaces** — for grouping without hierarchy.
...
[board]
columns = [ ... ]       # { id, label } in order; ids are what tasks store
                        # `backlog`/`queued`/`doing` are also the send rule's ids
default_agent = "pi"
...
notify_on_block = true         # a toast the first time an agent blocks on you
...
a blocked agent, a closed workspace, and an agent that exited.
